...
f_update_log_file <- function(log_file, name_file, new_name,
  ...
  data <- data.frame(old_name = name_file,
                    ...
                    type_survey = infos[6],
                    year_survey = date_survey,
                    month_survey =infos[4],
                    is_issue = issue,
...
check_and_generate_folder <- function( name){
  if (file.exists(paste(name, sep='')) == FALSE){
    dir.create(file.path(paste( name, sep='')))
  }
}
...
f_save_smart <- function(smart_folder, output_folder, file, new_name,
  ...
  if(issue == TRUE){
    ...
    file.copy(paste(smart_folder, file, sep = ""),
              folder_name)
    write.table(nutrition_data, file = paste(folder_name, '/', new_name, 'nutrition_data.csv', sep=""),
                append = FALSE, sep=',', col.names=TRUE, row.names = FALSE)
    write.table(mortality_data, file = paste(folder_name,'/', new_name, 'mortality_data.csv', sep=""),
                append = FALSE, sep=',', col.names=TRUE, row.names = FALSE)
    write.table(clusters_data, file = paste(folder_name, '/', new_name, 'clusters_data.csv', sep=""),
                append = FALSE, sep=',', col.names=TRUE, row.names = FALSE)
  }
